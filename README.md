...
📋 진행 사항
  * Delaunay Triangle 의 정점데이터 추가 시 4번째 혹은 그 이후부터 힙 오버플로우 발생하는 현상을 수정했습니다.
    * 현상 : 삼각형을 구성할 segment의 내각이 예각일 때, 4번째 정점부터는 도형의 형태에 따라 추가 segment를 생성할 수도 있음.
    * 해결 : 
    * 분석 : 처음에는 단순히 입력 정점 데이터의 잘못된 배열 접근으로 생각.->실제로도 코드상의 오류가 있었고 이후 '우연히 연속으로' 둔각으로 정점을 생성하면서 문제가 발생하지 않았음.
    * Triangle Library에 대한 이해도 부족으로 발생한 오류로, 내부적으로 기존에 이해한 것보다 많은 종류의 데이터를 생성해, 모든 malloc데이터에 대해 free를 호출하지 않아 발생.
