...
HRESULT CFlyingCameraController::Initialize(void* pArg)
{
	...
	m_pTransform->RotateYAxisFixed(Vec3(90.0f, 0.0f, 0.0f));
	m_pTransform->Translate(Vec3(0.0f, 900.0f, 0.0f));

	m_fLinearSpeed = 100.0f;
	m_vAngularSpeed = Vec3(180.0f, 180.0f, 180.0f);
...
{
	_float fSpeed = m_fLinearSpeed;

	if (KEY_PRESSING(KEY::CTRL))
		fSpeed *= 0.1f;

	if (KEY_PRESSING(KEY::W) || KEY_DOWN(KEY::W))
		m_pTransform->Translate(fTimeDelta * fSpeed * m_pTransform->GetForward());

	if (KEY_PRESSING(KEY::A) || KEY_DOWN(KEY::A))
		m_pTransform->Translate(-fTimeDelta * fSpeed * m_pTransform->GetRight());
	...
	if (KEY_PRESSING(KEY::Q) || KEY_DOWN(KEY::Q))
		m_pTransform->Translate(fTimeDelta * fSpeed * m_pTransform->GetUp());
	...
	if (dwMouseMove = m_pGameInstance->Get_DIMouseMove(DIMS_X))
	{
		if (m_pGameInstance->Mouse_Pressing(DIM_RB))
			m_pTransform->Translate(dwMouseMove * fTimeDelta * -m_pTransform->GetRight() * 12.0f);
	}

	if (dwMouseMove = m_pGameInstance->Get_DIMouseMove(DIMS_Y))
	{
		if (m_pGameInstance->Mouse_Pressing(DIM_RB))
			m_pTransform->Translate(dwMouseMove * fTimeDelta * m_pTransform->GetUp() * 12.0f);
	}
}
