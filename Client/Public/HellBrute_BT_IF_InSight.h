...
class CHellBrute_BT_IF_InSight : public CBT_Decorator
{
	using Super = CBT_Decorator;
private:
	CHellBrute_BT_IF_InSight() = default;
	CHellBrute_BT_IF_InSight(const CHellBrute_BT_IF_InSight& rhs) = delete;
	virtual ~CHellBrute_BT_IF_InSight() = default;
...
public:
	static	CHellBrute_BT_IF_InSight* Create(CGameObject* pGameObject, CBehaviorTree* pBehaviorTree, CMonoBehaviour* pController, DecoratorType eDecoratorType)
	{
		CHellBrute_BT_IF_InSight* pInstance = new CHellBrute_BT_IF_InSight;

		if (FAILED(pInstance->Initialize(pGameObject, pBehaviorTree, pController, eDecoratorType)))
		{
			MSG_BOX("Failed to Created : CHellBrute_BT_IF_InSight");
