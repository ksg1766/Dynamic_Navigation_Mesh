...
class CGoblin_BT_IF_InSight : public CBT_Decorator
{
	using Super = CBT_Decorator;
private:
	CGoblin_BT_IF_InSight() = default;
	CGoblin_BT_IF_InSight(const CGoblin_BT_IF_InSight& rhs) = delete;
	virtual ~CGoblin_BT_IF_InSight() = default;
...
public:
	static	CGoblin_BT_IF_InSight* Create(CGameObject* pGameObject, CBehaviorTree* pBehaviorTree, CMonoBehaviour* pController, DecoratorType eDecoratorType)
	{
		CGoblin_BT_IF_InSight* pInstance = new CGoblin_BT_IF_InSight;

		if (FAILED(pInstance->Initialize(pGameObject, pBehaviorTree, pController, eDecoratorType)))
		{
			MSG_BOX("Failed to Created : CGoblin_BT_IF_InSight");
